...
public class QuadTree {
	 ...
	 */
	public PostalCode findClosest(Coordinate targetCoord) {
		...
		if(root.containsCoordinate(targetCoord)){
			if (root.ne!=null && root.ne.containsCoordinate(targetCoord)){
			   ...
			   findClosest(targetCoord);
				...
				for(int i=0;i<root.getArray().size();i++){
					
					calcDistance= root.getArray().get(i).getCoordinate().distanceTo(targetCoord);
					
					if(calcDistance < closeDistance){
						closeDistance = calcDistance;
						pc = root.getArray().get(i);
					}
				} 
			}
		}
  ...
  // Beautiful illustration of the POST-ORDER RECURSIVE ALGORITHM
	...
	private class QuadNode {
		...
		public boolean add(PostalCode postalCode) {
			...
			if (postalCodeCollection == null) { // if true, INTERNAL node which does not accept PostalCode objects here.
				...
				if (postalCodeCollection.size() < maxItemsBeforeSplit) {
					postalCodeCollection.add(postalCode); // ONLY POSIBLE place in code where addition to the postalCodeCollection takes place, thus the increment to size is only here.
					++size; // instance field of the outer class BinarySearchTree. It is accessible because this class is "non-static."
					return true;
				} else {
					...
					for (PostalCode existingPostalCode : postalCodeCollection) {
						// The following call will distribute the existingPostalCode objects into the correct child QuadNode objects (ne, se, sw, nw).
						addToCorrectQuadrant(existingPostalCode); // no need to return from these calls, since the existingPostalCode MUST already be within bounds.
					}
					postalCodeCollection = null;
					return addToCorrectQuadrant(postalCode);
				}
			}
		}
		...
		public ArrayList<PostalCode> getArray(){
			return (ArrayList<PostalCode>) postalCodeCollection;
		}

		@Override
		public String toString() {
			return "something"; // TODO change this method.
		}
